...
namespace Ogre 
{
	...
	//---------------------------------------------------------------------
	void D3D9Texture::lockDeviceAccess()
	{		
		// Lock this texture device access.
		OGRE_LOCK_RECURSIVE_MUTEX(mDeviceAccessMutex);
	
		// Lock surfaces of each mip level.
		for(unsigned int i = 0; i < mSurfaceList.size(); ++i)
		{
			D3D9HardwarePixelBuffer* pixelBuffer = static_cast<D3D9HardwarePixelBuffer*>(mSurfaceList[i].get());

			pixelBuffer->lockDeviceAccess();			
		}			
	}

	//---------------------------------------------------------------------
	void D3D9Texture::unlockDeviceAccess()
	{			
		// Unlock surfaces of each mip level.
		for(unsigned int i = 0; i < mSurfaceList.size(); ++i)
		{
			D3D9HardwarePixelBuffer* pixelBuffer = static_cast<D3D9HardwarePixelBuffer*>(mSurfaceList[i].get());

			pixelBuffer->unlockDeviceAccess();			
		}

		// Unlock this texture device access.
